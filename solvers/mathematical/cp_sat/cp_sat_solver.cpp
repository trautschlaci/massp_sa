...
void CPSatSolver::solve_problem(OutputData& outputData, const std::string& log_file_name, double time_limit) const
{
    ...
    vector<vector<vector<BoolVar>>> peds(input_data.num_emps,
                                  vector<vector<BoolVar>>(input_data.num_periods+1,
                                          vector<BoolVar>(0)));
    for (int e = 0; e < input_data.num_emps; e++) {
        for (int d = 0; d < input_data.num_periods+1; d++) {
            for (int s = 0; s < sd[d].size(); s++) {
                peds[e][d].push_back(cp_model.NewBoolVar());
            }
        }
    }

    vector<vector<vector<BoolVar>>> qedl(input_data.num_emps,
                          vector<vector<BoolVar>>(input_data.num_periods+1,
                                  vector<BoolVar>(0)));
    for (int e = 0; e < input_data.num_emps; e++) {
        for (int d = 0; d < input_data.num_periods+1; d++) {
            for (int l = 0; l < L_set.size(); l++) {
                qedl[e][d].push_back(cp_model.NewBoolVar());
            }
        }
    }
    ...
    vector<vector<IntVar>> yia(num_horizon_intervals,
                          vector<IntVar>(input_data.num_tasks));
    for (int i = 0; i < num_horizon_intervals; i++) {
        for (int t = 0; t < input_data.num_tasks; t++) {
            yia[i][t] = cp_model.NewIntVar({0, input_data.num_emps});
        }
    }
    ...
    for (int t = 0; t < input_data.num_emps; t++) {
        for (int d = 0; d < input_data.num_periods+1; d++) {
            ted[t][d] = cp_model.NewIntVar({0, num_all_intervals});
        }
    }
    ...
    for (int t = 0; t < input_data.num_emps; t++) {
        for (int d = 0; d < input_data.num_periods+1; d++) {
            ned[t][d] = cp_model.NewIntVar({0, 40});
        }
    }
    ...
    for (int e = 0; e < input_data.num_emps; e++) {
        for (int d = 0; d < input_data.num_periods+1; d++) {
            ...
            cp_model.AddEquality(lhs, ted[e][d]);
        }
    }
    ...
    for (int e = 0; e < input_data.num_emps; e++) {
        for (int d = 0; d < input_data.num_periods+1; d++) {
            ...
            cp_model.AddEquality(lhs, ned[e][d]);
        }
    }
    ...
    for (int i = 0; i < num_horizon_intervals; i++) {
        for (int a = 0; a < input_data.num_tasks; a++)
        {
            ...
            cp_model.AddGreaterOrEqual(lhs, std::get<0>(input_data.cover_requirements[i+24][a]));
            cp_model.AddLessOrEqual(lhs, std::get<1>(input_data.cover_requirements[i+24][a]));
        }
    }
    ...
    for (int i = 0; i < num_horizon_intervals; i++) {
        for (int a = 0; a < input_data.num_tasks; a++) {
            auto cost = cp_model.NewIntVar({0, input_data.num_emps * input_data.num_emps});
            cp_model.AddMultiplicationEquality(cost, {yia[i][a], yia[i][a]});
            objective +=  cost;
        }
    }
